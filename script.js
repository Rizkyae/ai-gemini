...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let apiURL = `https://generativelanguage.googleapis.com/v1beta/models/${currentModel}:generateContent?key=${apiKey}`; // URL API dinamis
    ...
    function renderChatBox() { chatBox.innerHTML = ''; const currentChat = chats.find(c => c.id === currentChatId); if (currentChat && currentChat.messages) { currentChat.messages.forEach(msg => { addMessageToDOM(msg.content, msg.sender, msg.type, msg.filePreview, msg.fileObject); }); } } // Perbaikan: tambahkan msg.fileObject
    function startNewChat() { currentChatId = null; userInput.value = ''; clearFilePreview(); renderChatBox(); renderHistory(); }
    function addMessageToData(content, sender, type = 'text', filePreview = null, fileObject = null) { let currentChat = chats.find(c => c.id === currentChatId); if (!currentChat) return; currentChat.messages.push({ sender, content, type, filePreview, fileObject }); saveChats(); } // Perbaikan: tambahkan fileObject
    ...
    function embedYouTubeLinks(text) {
        ...
        return text.replace(youtubeRegex, (match, videoId) => {
            return `<div class="youtube-embed-wrapper"><iframe src="https://www.youtube.com/embed/${videoId}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>`; // Perbaikan URL YouTube
        });
    }
    ...
    // Fungsi getAIResponse diperbarui untuk menggunakan currentModel
    async function getAIResponse(parts) { 
        try { 
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${currentModel}:generateContent?key=${apiKey}`, { // Gunakan currentModel
                method: 'POST', 
    ...
    async function handleSend() {
        ...
        addMessageToData(userMessage, 'user', 'text', filePreviewForDOM, attachedFile); // Kirim fileObject ke addMessageToData
        
        const parts = [];
        let personaPrompt = `React as Riski, your AI bestie. Your personality is super chill, helpful, and you talk like a true Gen Z from Indonesia. Use casual Indonesian and mix in English slang (e.g., 'literally', 'spill', 'no cap', 'YGY', 'bestie'). Use emojis.`;
